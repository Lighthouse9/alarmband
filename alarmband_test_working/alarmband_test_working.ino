...
// A small helper
void bluefruit_error(const __FlashStringHelper*err) {
  Serial.println(err);
  while (1);
}
//EDNFROM
...
void setup()
{
  ...
  if ( !ble.begin(true) )
  {
    bluefruit_error(F("Couldn't find Bluefruit, make sure it's in CoMmanD mode & check wiring?"));
  }
  Serial.println( F("OK!") );
  ...
  if ( 1 )
  {
    ...
    if ( ! ble.factoryReset() ) {
      bluefruit_error(F("Couldn't factory reset"));
    }
  }
...
//  //BLUEFRUIT
//  ble.print("AT+BLEUARTTX=");
//  ble.print(F("Read accel, temp and gyro, error = "));
//  ble.println(error, DEC);
//  //END
...
#define SWAP(x,y) swap = x; x = y; y = swap
  ...
  //BLUEFRUIT
  ble.print("AT+BLEUARTTX=");
  ble.print(F("accel x,y,z: "));
  ble.print(accel_t_gyro.value.x_accel, DEC);
  ble.print(F(", "));
  ble.print(accel_t_gyro.value.y_accel, DEC);
  ble.print(F(", "));
  ble.print(accel_t_gyro.value.z_accel, DEC);
  ble.println(F(""));
  //END
  //total = abs(oax-accel_t_gyro.value.x_accel) + abs(oay-accel_t_gyro.value.y_accel)+abs(oaz-accel_t_gyro.value.z_accel);
  total = accel_t_gyro.value.x_accel + accel_t_gyro.value.y_accel + accel_t_gyro.value.z_accel -16384;
 oax = accel_t_gyro.value.x_accel;
 oay = accel_t_gyro.value.y_accel;
 oaz = accel_t_gyro.value.z_accel;
...
//  //END
  ...
  //BLUEFRUIT
  ble.print("AT+BLEUARTTX=");
  ble.println(F("gyro x,y,z : "));
  ble.print("AT+BLEUARTTX=");
  ble.print(accel_t_gyro.value.x_gyro, DEC);
  ble.print(F(", "));
  ble.print(accel_t_gyro.value.y_gyro, DEC);
  ble.print(F(", "));
  ble.print(accel_t_gyro.value.z_gyro, DEC);
  ble.print(F(", "));
  ble.println(F("                                                            "));
  //ble.println(F(" "));
  //END
 total += abs(ogx-accel_t_gyro.value.x_gyro) + abs(ogy-accel_t_gyro.value.y_gyro) + abs(ogz - accel_t_gyro.value.z_gyro);
 ogx = accel_t_gyro.value.x_gyro;
 ogy = accel_t_gyro.value.y_gyro;
 ogz = accel_t_gyro.value.z_gyro;
 if(total < 275){
